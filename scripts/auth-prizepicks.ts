...
const STORAGE_FILE = path.join(STORAGE_DIR, 'prizepicks.storage.json');

async function main() {
  console.log('========================================');
  console.log('PrizePicks Authentication Script');
  console.log('========================================\n');
  console.log(`Storage file: ${STORAGE_FILE}`);
  console.log('\nA headed browser will open. Please log in to PrizePicks.');
  ...
  await page.goto(PRIZEPICKS_URL, { waitUntil: 'networkidle' });
  ...
  try {
    ...
    console.log('  PRIZEPICKS_STORAGE_STATE=prizepicks.storage.json');
  } catch (error) {
