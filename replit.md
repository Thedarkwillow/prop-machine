...
**Stale Prop Filtering (Nov 22, 2025)**: Implemented three-layer system to prevent old games from displaying: (1) **API-level filtering**: `getActivePropsWithLineMovement()` in storage layer excludes props with `gameTime` older than 1 hour using parameterized JavaScript cutoff; (2) **Automatic deactivation**: `deactivateExpiredProps()` marks old props inactive during each refresh cycle (gated to only run when new props are successfully created to preserve data during rate-limits); (3) **Synthetic fixture IDs**: PrizePicks props now generate fixture IDs formatted as `prizepicks_{sport}_{team}_{YYYYMMDD_HHMM}` for fixture-level deactivation. Prop refresh interval reduced from 30 minutes to 5 minutes for 6x faster updates. Known limitation: PrizePicks props missing team data may experience fixture ID collisions when games start at identical times.

**Real-Time ML Analysis Integration (Nov 22, 2025)**: Successfully integrated inline ML analysis into OpticOdds SSE streaming pipeline, replacing hardcoded 50% placeholder confidence scores with real ESPN-powered statistical analysis. System now analyzes every prop in real-time during ingestion, producing **varied confidence scores (53%-73% observed range)** based on actual player performance data. Implementation features: (1) **15-second timeout** for ESPN API calls to prevent blocking; (2) **Graceful degradation** - falls back to 50% confidence on timeout/failure; (3) **Database deadlock resilience** - exponential backoff retry logic (100ms, 200ms, 400ms) handles concurrent write conflicts from parallel NBA/NFL/NHL streams; (4) **Comprehensive stat support** - handles combo props (PRA, Pts+Rebs, etc.), quarter-specific props, and advanced stats through propAnalysisService. Confidence scores now reflect genuine ML-derived probabilities rather than uniform placeholders, significantly improving betting intelligence accuracy. Known limitation: Special characters in player names (e.g., "Nikola Jokić" with accent) may cause intermittent lookup failures, though many succeed.
