...
**Real-Time Streaming Architecture**: The platform integrates OpticOdds API with production-ready SSE streaming for live odds updates and automated bet settlement. The `OpticOddsStreamService` handles real-time odds streaming with fixture caching for team name resolution, while `OpticOddsResultsStreamService` processes live game results and automatically grades bets with bankroll updates. The streaming system features: (1) **Prop Deduplication** - upsertProp method prevents duplicate props on SSE updates, matching by sport+platform+player+stat+line+direction; (2) **Fixture Caching** - fetches and caches team names from active fixtures at stream start, eliminating "TBD" placeholders; (3) **Automatic Reconnection** - resumes from last_entry_id after disconnections with exponential backoff (max 5 attempts); (4) **Admin-Only Access** - all streaming control endpoints protected by requireAdmin middleware; (5) **Atomic Bet Settlement** - settleBetWithBankrollUpdate atomically updates bet status and user bankroll in transactions when games complete; (6) **Parallel Stream Management** - supports multiple concurrent streams for different sports/sportsbooks with independent lifecycle management. All API integrations feature parallel data fetching, proper sportsbook attribution, and robust error handling with batch-level retry logic.
