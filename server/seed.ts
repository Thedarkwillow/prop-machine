...
export async function refreshProps() {
  try {
    ...
    // Deactivate all existing active props
    const activeProps = await storage.getAllActiveProps();
    for (const prop of activeProps) {
      await storage.deactivateProp(prop.id);
    }
    console.log(`Deactivated ${activeProps.length} old props`);

    // Create fresh props with current game times
    ...
    console.log(`✅ Refreshed ${props.length} props with current game times`);
    return { success: true, count: props.length };
  } catch (error) {
